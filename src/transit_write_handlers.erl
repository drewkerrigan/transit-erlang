...
handler_([{_,_}|_]) ->
  #write_handler{tag = fun map_tag/1, rep = fun map_rep/1, string_rep = fun map_string_rep/1};
handler_(Data) when is_list(Data) ->
  #write_handler{tag = fun array_tag/1, rep = fun array_rep/1, string_rep = fun array_string_rep/1};
handler_(Data) when is_binary(Data) ->
...
handler_(nan) -> special_number();
handler_(infinity) -> special_number();
handler_(neg_infinity) -> special_number();
handler_({kw, _}) ->
  #write_handler {
